...
Bevy core has **no state machines, blend trees, IK, or root motion** — Unreal's AnimGraph ships all of these. Rather than depend on the community [`bevy_animation_graph`](https://github.com/mbrea-c/bevy_animation_graph) and [`bevy_mod_inverse_kinematics`](https://github.com/Kurble/bevy_mod_inverse_kinematics) — ecosystem crates that lag major Bevy releases and can break the `--all-features` CI gate — we implement the pieces we need **in-house** ([ADR-0003](../../../architecture/decisions/0003-in-house-ik-and-vat.md), following [ADR-0001](../../../architecture/decisions/0001-engine-crate-family-and-features.md)): an analytic **two-bone** limb solver (pole-controlled) and a general **FABRIK** chain solver (`engine-anim::ik`), plus the crowd **VAT** frame/index math (`engine-anim::vat`). All are pure and bit-deterministic where shared with the server sim. **FSM/blend-space authoring** layers on top and is not yet built. **Root motion has no standard solution** — for a server-authoritative game, motion is driven by [predicted](../../client/prediction-core/README.md) transforms anyway, so root-motion authority stays server-side; the client animates *to* it. Where the engine is already strong: glTF skinned animation + GPU skinning + additive/masked blending are solid out of the box.
